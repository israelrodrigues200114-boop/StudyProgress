...
def page_inicio(all_data):
    ...
    pendencias_total = len(pend_antigas)
...
def page_simulados():
    ...
    with tab1:
        ...
        opcoes_inicio = [1, 46, 91, 136]
        chave_inicio = f"inicio_questoes_{prova['ID_Prova']}"
        inicio_atual = st.session_state.get(chave_inicio, 1)
        if inicio_atual not in opcoes_inicio:
            inicio_atual = 1
        c1, c2 = st.columns([1, 2])
        with c1:
            inicio = st.selectbox("A numeração começa em", opcoes_inicio, index=opcoes_inicio.index(inicio_atual), key=f"select_inicio_{prova['ID_Prova']}")
        with c2:
            st.caption(f"Essa prova será respondida como **Q{inicio} a Q{inicio + total - 1}**. Escolha 1, 46, 91 ou 136 conforme o bloco da prova.")
        st.session_state[chave_inicio] = inicio

        st.markdown(f"### {prova_nome}")
        st.caption(f"Área: {prova['Área']} • Questões: {inicio} a {inicio + total - 1}")
        ...
        questoes = list(range(inicio, inicio + total))
        with st.form(f"respostas_{prova['ID_Prova']}_{inicio}"):
            respostas = {}
            if modo_lista:
                for bloco_pos in range(0, total, 15):
                    bloco = questoes[bloco_pos:bloco_pos + 15]
                    st.markdown(f"#### Questões {bloco[0]} a {bloco[-1]}")
                    for q in bloco:
                        c1, c2 = st.columns([.30, .70])
                        with c1:
                            st.markdown(f"**Questão {q}**")
                        with c2:
                            respostas[q] = st.radio("Alternativa", ALT_OPTIONS, key=f"resp_{prova['ID_Prova']}_{q}", horizontal=True, label_visibility="collapsed")
            else:
                for linha_pos in range(0, total, 5):
                    bloco = questoes[linha_pos:linha_pos + 5]
                    cols = st.columns(5)
                    for offset, q in enumerate(bloco):
                        with cols[offset]:
...
def page_banco_erros():
    ...
    with tab1:
        ...
        for _, row in due.iterrows():
            ...
            c1, c2 = st.columns(2)
            with c1:
                if st.button("Revisado", key=f"ok_err_{row['ID_Erro']}"):
                    avancar_revisao_erro(row["ID_Erro"], acertou=True); st.rerun()
            with c2:
                if st.button("Preciso rever mais vezes", key=f"duv_err_{row['ID_Erro']}"):
...
def page_desempenho(all_data):
    ...
    if not correcoes.empty:
        correcoes_perf = correcoes.copy()
        correcoes_perf["Resultado"] = correcoes_perf["Resultado"].fillna("").astype(str).str.strip()
        provas_total = len(correcoes_perf)
        provas_acertos = int((correcoes_perf["Resultado"] == "Acertei").sum())
        provas_erros = int((correcoes_perf["Resultado"] == "Errei").sum())
        provas_taxa = round(provas_acertos / provas_total * 100, 1) if provas_total else 0
    else:
        provas_total = provas_acertos = provas_erros = 0
        provas_taxa = 0
    c1,c2,c3,c4=st.columns(4)
    ...
    st.write("")
    with st.container(border=True):
        card_header("Desempenho nas provas por área","Acertos, erros e aproveitamento das provas corrigidas.")
        if correcoes.empty:
            st.info("Ainda não existem provas corrigidas para mostrar o desempenho por área.")
        else:
            area_perf=(correcoes_perf.groupby("Área",as_index=False).agg(Provas=("Prova","nunique"),Questões=("Questão","count"),Acertos=("Resultado",lambda s:int((s=="Acertei").sum())),Erros=("Resultado",lambda s:int((s=="Errei").sum()))))
            area_perf["Taxa de acertos"]=(area_perf["Acertos"]/area_perf["Questões"]*100).round(1)
            area_perf=area_perf.sort_values("Taxa de acertos",ascending=False)
            st.dataframe(area_perf.rename(columns={"Provas":"Provas corrigidas","Taxa de acertos":"Taxa de acertos (%)"}),use_container_width=True,hide_index=True)
            if px is not None:
                fig=px.bar(area_perf,x="Área",y="Taxa de acertos",color="Área",color_discrete_map={a:get_color_for_area(a) for a in area_perf["Área"]},hover_data=["Questões","Acertos","Erros","Provas"])
                fig=apply_plot_style(fig,330); fig.update_layout(showlegend=False,xaxis_title="",yaxis_title="Taxa de acertos (%)"); fig.update_yaxes(range=[0,100]); st.plotly_chart(fig,use_container_width=True)
            pc1,pc2,pc3,pc4=st.columns(4)
            with pc1: metric_card("Provas corrigidas",provas_total,"todas as áreas")
            with pc2: metric_card("Acertos em provas",provas_acertos,f"{provas_taxa}% de aproveitamento")
            with pc3: metric_card("Erros em provas",provas_erros,"questões erradas")
            with pc4: metric_card("Áreas avaliadas",area_perf["Área"].nunique(),"com provas corrigidas")
    st.write("")
    left,right=st.columns(2)
    with left:
        with st.container(border=True):
            ...
            if evo.empty: st.info("Sem dados suficientes.")
                ...
                fig=px.area(evo,x="Data",y="Tempo (h)",markers=True); fig.update_traces(line_color="#8B5CF6",fillcolor="rgba(244,114,182,.18)",marker=dict(size=8,color="#F6C453",line=dict(width=2,color="#8B5CF6"))); fig=apply_plot_style(fig,350); fig.update_layout(xaxis_title="",yaxis_title="Horas"); st.plotly_chart(fig,use_container_width=True)
            else: st.line_chart(evo,x="Data",y="Tempo (h)",height=320)
    with right:
        with st.container(border=True):
            ...
            if subj.empty: st.info("Sem dados suficientes.")
            elif px is not None:
                colors={m:SUBJECT_COLORS.get(m,"#64748B") for m in subj["Matéria"]}; fig=px.bar(subj,x="Matéria",y="Tempo (h)",color="Matéria",color_discrete_map=colors); fig=apply_plot_style(fig,350); fig.update_layout(showlegend=False,xaxis_title="",yaxis_title="Horas"); st.plotly_chart(fig,use_container_width=True)
            else: st.bar_chart(subj,x="Matéria",y="Tempo (h)",height=320)
    left,right=st.columns(2)
    with left:
        with st.container(border=True):
            card_header("Acertos por matéria")
            if all_data.empty or int(all_data["Exercícios"].sum())==0: st.info("Sem exercícios registrados.")
            else:
                acc=all_data.groupby("Matéria",as_index=False).agg({"Exercícios":"sum","Acertos":"sum"}); acc=acc[acc["Exercícios"]>0]; acc["Taxa"]=(acc["Acertos"]/acc["Exercícios"]*100).round(1)
                if px is not None and not acc.empty:
                    fig=px.bar(acc.sort_values("Taxa",ascending=False),x="Matéria",y="Taxa",color="Matéria",color_discrete_map={m:SUBJECT_COLORS.get(m,"#64748B") for m in acc["Matéria"]}); fig=apply_plot_style(fig,350); fig.update_layout(showlegend=False,xaxis_title="",yaxis_title="Taxa (%)"); fig.add_hline(y=70,line_dash="dot",line_color="#F6C453",annotation_text="Meta 70%",annotation_position="top left"); st.plotly_chart(fig,use_container_width=True)
                else: st.dataframe(acc,use_container_width=True,hide_index=True)
    with right:
        with st.container(border=True):
            card_header("Erros por tipo")
            if erros.empty or erros["Tipo_Erro"].replace("",pd.NA).dropna().empty: st.info("Sem erros registrados.")
            else:
                grp=erros.groupby("Tipo_Erro",as_index=False).size().rename(columns={"size":"Total"})
                if px is not None:
                    fig=px.pie(grp,names="Tipo_Erro",values="Total",hole=.58,color_discrete_sequence=["#8B5CF6","#F472B6","#F6C453","#14B8A6","#FB7185","#38BDF8"]); fig.update_traces(textposition="inside",textinfo="percent+label",marker=dict(line=dict(color="#FFF7ED",width=3))); fig=apply_plot_style(fig,350); fig.update_layout(legend_title_text="",annotations=[dict(text="Erros",x=.5,y=.5,font_size=18,showarrow=False,font_color="#111827")]); st.plotly_chart(fig,use_container_width=True)
                else: st.bar_chart(grp,x="Tipo_Erro",y="Total",height=320)
