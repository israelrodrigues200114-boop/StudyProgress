...
def extract_exam_year(value):
    """Extrai o ano da prova a partir do nome, ex.: 'ENEM 2009 Natureza' -> 2009."""
...
def page_simulados():
    ...
    with tab1:
        ...
        inicio_questoes = st.selectbox(
            "Numeração da prova — primeira questão",
            [1, 46, 91, 136],
            index=0,
            key=f"inicio_questoes_{prova['ID_Prova']}",
        )
        fim_questoes = inicio_questoes + total - 1
        st.markdown(f"### {prova_nome}")
        st.caption(f"Área: {prova['Área']} • Questões: {inicio_questoes} a {fim_questoes}")
        ...
        with st.form(f"respostas_{prova['ID_Prova']}"):
            respostas = {}
            questoes = range(inicio_questoes, fim_questoes + 1)
            if modo_lista:
                questoes_lista = list(questoes)
                for bloco_inicio in range(0, len(questoes_lista), 15):
                    bloco = questoes_lista[bloco_inicio:bloco_inicio + 15]
                    bloco_fim = bloco[-1]
                    st.markdown(f"#### Questões {bloco[0]} a {bloco_fim}")
                    for q in bloco:
                ...
                questoes_lista = list(questoes)
                for linha_inicio in range(0, len(questoes_lista), 5):
                    cols = st.columns(5)
                    for offset, q in enumerate(questoes_lista[linha_inicio:linha_inicio + 5]):
                        with cols[offset]:
...
def page_banco_erros():
    ...
    # Filtro por ano da prova — aplica-se às três abas do Banco de Erros.
    erros = erros.copy()
    erros["_Ano_Prova"] = erros["Prova"].apply(extract_exam_year)

    anos = sorted(
        [int(a) for a in erros["_Ano_Prova"].dropna().unique()],
        reverse=True
    )

    filtro_col, resumo_col = st.columns([1, 2])
    with filtro_col:
        opcoes_ano = ["Todos os anos"] + [str(ano) for ano in anos]
        ano_selecionado = st.selectbox(
            "Ano da prova",
            opcoes_ano,
            key="banco_erros_filtro_ano"
        )

    if ano_selecionado != "Todos os anos":
        erros = erros[erros["_Ano_Prova"] == int(ano_selecionado)].copy()

    with resumo_col:
        if ano_selecionado == "Todos os anos":
            st.caption(f"Mostrando {len(erros)} erro(s) de todos os anos.")
        else:
            st.caption(f"Mostrando {len(erros)} erro(s) de {ano_selecionado}.")
    ...
    with tab1:
        due = erros[
            (erros["Status_Revisao"] != "Concluída") &
            (erros["Proxima_Revisao_dt"] <= today_ts())
        ].sort_values("Proxima_Revisao_dt")
        ...
        for _, row in due.iterrows():
            ...
            with c1:
                if st.button("Revisado", key=f"ok_err_{row['ID_Erro']}"):
                    avancar_revisao_erro(row["ID_Erro"], acertou=True)
                    st.rerun()
            with c2:
                if st.button("Preciso rever mais vezes", key=f"duv_err_{row['ID_Erro']}"):
                    avancar_revisao_erro(row["ID_Erro"], acertou=False)
                    st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab2:
        ...
        st.dataframe(
            show.drop(columns=["Proxima_Revisao_dt", "_Ano_Prova"], errors="ignore"),
    ...
    with tab3:
        ...
        if rev.empty:
            ...
            st.dataframe(
                rev.drop(columns=["Proxima_Revisao_dt", "_Ano_Prova"], errors="ignore"),
                use_container_width=True,
                hide_index=True
            )
...
def page_desempenho(all_data):
    ...
    st.write("")
    with st.container(border=True):
        card_header("Desempenho nas provas por área", "Provas corrigidas, acertos, erros e taxa de acertos.")
        if correcoes.empty:
            st.info("Nenhuma prova corrigida ainda.")
        else:
            provas_area = correcoes.groupby("Área", as_index=False).agg(
                Provas=("Prova", "nunique"),
                Questões=("Questão", "count"),
                Acertos=("Resultado", lambda s: (s.astype(str).str.strip().str.lower() == "acertei").sum()),
                Erros=("Resultado", lambda s: (s.astype(str).str.strip().str.lower() != "acertei").sum()),
            )
            provas_area["Taxa de acertos"] = (
                provas_area["Acertos"] / provas_area["Questões"] * 100
            ).round(1)
            provas_area = provas_area.sort_values("Taxa de acertos", ascending=False)
            st.dataframe(
                provas_area,
                use_container_width=True,
                hide_index=True,
                column_config={
                    "Provas": st.column_config.NumberColumn("Provas"),
                    "Questões": st.column_config.NumberColumn("Questões"),
                    "Acertos": st.column_config.NumberColumn("Acertos"),
                    "Erros": st.column_config.NumberColumn("Erros"),
                    "Taxa de acertos": st.column_config.NumberColumn("Taxa de acertos", format="%.1f%%"),
                },
            )
            if px is not None and not provas_area.empty:
                fig = px.bar(
                    provas_area,
                    x="Área",
                    y="Taxa de acertos",
                    color="Área",
                    color_discrete_map={a: AREA_COLORS.get(a, "#64748B") for a in provas_area["Área"]},
                )
                fig = apply_plot_style(fig, 350)
                fig.update_layout(showlegend=False, xaxis_title="", yaxis_title="Taxa de acertos (%)")
                st.plotly_chart(fig, use_container_width=True)
