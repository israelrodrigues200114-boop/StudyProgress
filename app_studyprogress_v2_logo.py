...
LOGO_FILENAME = APP_DIR / "studyprogress_logo.png"

def logo_data_uri() -> str:
    """Retorna o logo oficial como data URI para uso no app."""
    for path in (LOGO_FILENAME, Path("/mnt/data/studyprogress_logo.png")):
        if path.exists():
            return "data:image/png;base64," + base64.b64encode(path.read_bytes()).decode("ascii")
    return ""

def logo_svg(size=54):
    size = int(size)
    uri = logo_data_uri()
    if uri:
        return (
            f'<img class="brand-logo-img" src="{uri}" alt="StudyProgress" '
            f'style="width:{size}px;height:{size}px;border-radius:18px;object-fit:cover;display:block;" />'
        )
    # fallback simples para não quebrar caso a imagem não exista
    return (
        f'<svg class="brand-icon" width="{size}" height="{size}" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img">'
        f'<defs><linearGradient id="spbg" x1="0%" y1="0%" x2="100%" y2="100%">'
        f'<stop offset="0%" stop-color="#8B5CF6"/><stop offset="52%" stop-color="#F472B6"/>'
        f'<stop offset="100%" stop-color="#F6C453"/></linearGradient></defs>'
        f'<rect x="1.25" y="1.25" width="61.5" height="61.5" rx="19" fill="url(#spbg)"/>'
        f'<text x="50%" y="54%" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="27" font-weight="900" fill="#FFF7ED">SP</text>'
        f'</svg>'
    )
...
st.markdown(
    ...
    .intro-splash {
        ...
        background:
            ...
            radial-gradient(circle at top right, rgba(244,114,182,.16), transparent 24%),
            linear-gradient(180deg, rgba(11,16,38,.96), rgba(21,27,61,.98));
    }
    ...
    .intro-progress span {
        ...
        background: linear-gradient(90deg, #8B5CF6, #F472B6 58%, #F6C453);
        animation: introBar .9s ease-out both;
    }
...
def logo_svg(size=54):
    size = int(size)
    return f'''
    <svg class="brand-icon" width="{size}" height="{size}" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img">
      <defs>
        ...
        <linearGradient id="sgrad" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <stop offset="100%" stop-color="#D7C8FF"/>
        </linearGradient>
      ...
      <rect x="1.25" y="1.25" width="61.5" height="61.5" rx="19" fill="url(#spbg)"/>
      <rect x="1.25" y="1.25" width="61.5" height="61.5" rx="19" fill="rgba(9,14,33,.10)"/>
      <path d="M14 45.5c6.5-3.8 13.7-5.5 20.4-5.5 7.5 0 13 1.5 15.6 3.1" fill="none" stroke="rgba(255,247,237,.78)" stroke-width="2.2" stroke-linecap="round"/>
      <path d="M15 49.2c6.4-3.6 13.4-5 19.8-5 7.3 0 12.8 1.4 15.2 2.8" fill="none" stroke="rgba(255,247,237,.52)" stroke-width="1.8" stroke-linecap="round"/>
      <text x="17.2" y="38.5" fill="url(#sgrad)" font-size="24" font-weight="900" font-family="Inter, Arial, sans-serif" letter-spacing="-2.5">S</text>
      <text x="32.5" y="38.5" fill="url(#pgrad)" font-size="24" font-weight="900" font-family="Inter, Arial, sans-serif" letter-spacing="-2.5">P</text>
      <path d="M41 14.5l1.8 4.2 4.2 1.8-4.2 1.8-1.8 4.2-1.8-4.2-4.2-1.8 4.2-1.8z" fill="#FFD66B"/>
      <path d="M50.2 24.2l.9 2.1 2.1.9-2.1.9-.9 2.1-.9-2.1-2.1-.9 2.1-.9z" fill="#F9A8D4"/>
      <path d="M47.4 33.6l.8 1.8 1.8.8-1.8.8-.8 1.8-.8-1.8-1.8-.8 1.8-.8z" fill="#C084FC"/>
    </svg>
    '''

def brand_block(title='StudyProgress', subtitle='Foco, provas e revisão'):
    logo = logo_svg(54)
    return f"""
    <div class="sidebar-brand">
        ...
        <div class="brand-stack">
            ...
            <div class="brand-subtitle">{html.escape(subtitle)}</div>
        </div>
    </div>
    """
...
def intro_block():
    ...
    <div class="intro-splash">
        <div class="intro-card">
            {logo_svg(72)}
            <div class="intro-title">StudyProgress</div>
            <div class="intro-sub">Organize. Estude. Evolua.</div>
            <div class="intro-progress"><span></span></div>
            <div class="intro-caption">Preparando seu painel...</div>
        </div>
    </div>
    """
...
def show_intro_splash():
    ...
    placeholder.markdown(intro_block(), unsafe_allow_html=True)
    time.sleep(0.95)
    placeholder.empty()
...
def page_inicio(all_data):
    show_topbar("Olá, Israel", "Seu painel do dia com foco, clareza e tudo no lugar.")
    ...
    st.markdown(
        ...
        <div class="hero">
            ...
            <p>Organize sua rotina de estudos, acompanhe seu progresso e mantenha teoria, provas e revisões sempre em dia.</p>
            <div class="study-art">
            ...
            if teoria_hoje.empty:
                ...
                for _, row in teoria_hoje.iterrows():
                    ...
                    task_card(
                        f"{horario} — {materia}",
                        row.get("Observacoes", ""),
                        subject_badge(materia),
                        get_color_for_subject(materia),
                    )
                    quick_nav_button(
                        "Registrar estudo",
                        "Adicionar",
            ...
            if provas_hoje.empty:
                ...
                for _, row in provas_hoje.iterrows():
                    ...
                    task_card(
                        row.get("Atividade", ""),
                        meta,
                        status_badge(row.get("Status", "Pendente")),
                        get_color_for_area(area),
                    )
        ...
        with st.container(border=True):
            ...
            if proximas_provas.empty and pend_antigas.empty and pend_erros_abertos.empty:
                ...
                for _, row in proximas_provas.iterrows():
                    task_card(
                        f"{format_date_br(row['Data_dt'])} — {row['Atividade']}",
                        row.get("Área", ""),
                        status_badge(row.get("Status", "Pendente")),
                        get_color_for_area(row.get("Área", "")),
                    )
                for _, row in pend_antigas.head(3).iterrows():
                    task_card(
                        f"{row['Matéria']} — {row['Conteúdo']}",
                        "pendência antiga",
                        subject_badge(row['Matéria']),
                        get_color_for_subject(row['Matéria']),
                    )
        ...
        with st.container(border=True):
            ...
            if rev_erros_hoje.empty and reviews_antigas.empty:
                ...
                for _, row in rev_erros_hoje.head(3).iterrows():
                    task_card(
                        f"Questão {row['Questão']} — {row['Prova']}",
                        row.get("Tipo_Erro", "erro"),
                        status_badge("Revisar hoje"),
                        get_color_for_area(row.get("Área", "")),
                    )
